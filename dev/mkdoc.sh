...
CLANG_FLAGS="-std=c++17 -DNOMINMAX -DCMAKE_BUILD_TYPE=Release -D_ALLOW_COMPILER_AND_STL_VERSION_MISMATCH ${@}"
...
for filepath in `find "$CPP_INCLUDE_DIR" -name "*.h" ! -name "types.h" ! -path "*macros*"`; do
  ...
  E:/ENVs/pymplib/python.exe "$PY_SCRIPT_PATH" -o="$output_path" $CLANG_FLAGS "$filepath" &
done
