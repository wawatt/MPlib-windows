...
class Planner:
    ...
    def FK(self, qpos: np.ndarray): #-> np.ndarray: #Pose:
        """
        Compute forward kinematics for the robot.

        :return: pose of the end-effector in world frame.
        """
        move_link_idx = self.link_name_2_idx[self.move_group]
        qpos = self.pad_move_group_qpos(qpos)
        self.pinocchio_model.compute_forward_kinematics(qpos)
        pose = self.pinocchio_model.get_link_pose(move_link_idx)
        return np.concatenate((pose.get_p(), pose.get_q()))
    
    def IK(
        self,
        goal_pose: np.ndarray, #Pose,
        ...
        *,
        ...
        if mask is None:
            mask = []
        goal_pose = Pose(goal_pose[0:3], goal_pose[4:])  # type: ignore
